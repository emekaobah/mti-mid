...
import { useTradeColumns } from "./column";
...
const SectorBreakdown = () => {
  ...
  const shouldFetchHsCodes = sector.sectorId !== "SECTOR_016";
  const { data: hsCodes } = useProductsBySector(
    shouldFetchHsCodes ? sector.sectorId : null
  );
  ...
  //     ) ?? []),
  // ];
  const hsCodeOptions = shouldFetchHsCodes
    ? [
        { label: "All", value: "all" },
        ...(hsCodes?.data
          ?.map((sector) => ({
